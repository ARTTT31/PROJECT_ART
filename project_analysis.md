...
- **Status:** ✅ **RESOLVED**
- **Detail:** `argon2-cffi>=23.1.0` has been added to `backend/requirements.txt`. Backend now starts and hashes passwords correctly using Argon2.
...
- **Status:** ✅ **RESOLVED**
- **Detail:** `SECRET_KEY` has been correctly documented in the root `.env` files and `.env.example`.

#### CRIT-04: Google OAuth Lacks ID Token Verification
- **Status:** ✅ **RESOLVED**
- **Detail:** Google OAuth validation has been configured and works seamlessly in the production deployment.
...
- **Status:** 🟡 **ACTIVE**
- **Detail:** Inline imports like `from app.services.audit_service import AuditService` still reside inside function bodies across endpoints (e.g. `profile.py` and `users.py`).

#### MED-02: Sidebar Uses `rounded-2xl` (16px) — DESIGN.md Specifies 12px
- **Status:** 🟡 **ACTIVE**
- **Detail:** Navigation items in `Sidebar.tsx` still use `rounded-2xl`.

#### MED-03: Duplicate Rate Limiter Instance
- **Status:** 🟡 **ACTIVE**
- **Detail:** A duplicate `Limiter` instance is still present in `auth.py`.

#### MED-04: `datetime.utcnow()` Used Instead of Timezone-Aware Pattern
- **Status:** ✅ **RESOLVED**
- **Detail:** Handled via custom helper `_utcnow()` in `security.py` and aligned database timestamps.

#### MED-05: No Content Security Policy (CSP) Headers
- **Status:** 🟡 **ACTIVE**
- **Detail:** CSP headers are still not explicitly configured in either the FastAPI middleware or Next.js next.config.js.

#### MED-06: Session Cleanup Not Automated
- **Status:** 🟡 **ACTIVE**
- **Detail:** The session cleanup script still needs to be run manually.

#### MED-07: Alembic `created_at`/`updated_at` Nullable Mismatch
- **Status:** 🟡 **ACTIVE**
- **Detail:** The discrepancy between database schema nullability and ORM model level checks on timestamp fields remains.

#### MED-08: Google Login Redirects to Port 3001 but Compose Exposes Port 3000
- **Status:** ✅ **RESOLVED**
- **Detail:** Port mappings have been aligned in the local startup configurations.

#### MED-09: Pydantic v1 Deprecation Warning
- **Status:** 🟡 **ACTIVE**
- **Detail:** Warnings regarding `@validator` are still generated when starting pytest.
...
- **Status:** 🟡 **ACTIVE** - Minor maintainability concern. No functional impact.
...
- **Status:** 🟡 **ACTIVE** - Redundant CSS. First block is less strict than the second.
...
- **Status:** 🟡 **ACTIVE** - Regression risk. Any widget change could break without detection.
...
- **Status:** 🟡 **ACTIVE** - Hard to diagnose production issues.
...
- **Status:** 🟡 **ACTIVE** (Note: Since the system has migrated towards a Docker-less development setup, this is lower priority.)
