...
def _parse_eppo_html(html: str) -> list[dict]:
    ...
    for img_file, key, name in IMAGE_MAP:
        if img_file in row_map:
            result.append({"key": key, "name": name, "price": row_map[img_file], "unit": "บาท/ลิตร"})
    return result
...
async def get_oil_prices():
    try:
        async with httpx.AsyncClient(timeout=10, follow_redirects=True) as client:
            response = await client.get(EPPO_OIL_URL, headers={"User-Agent": "Mozilla/5.0"})

        if response.status_code == 200:
            prices = _parse_eppo_html(response.text)
            if prices:
                today = datetime.date.today().strftime("%d/%m/%Y")
                return {"success": True, "prices": prices, "update_date": today, "source": "EPPO"}
...
def _fallback_prices():
    ...
    return {
        "success": True,
        "prices": [
            {"key": "gasohol_95",  "name": "แก๊สโซฮอล์ 95",  "price": 43.10, "unit": "บาท/ลิตร"},
            {"key": "gasohol_91",  "name": "แก๊สโซฮอล์ 91",  "price": 42.73, "unit": "บาท/ลิตร"},
            {"key": "gasohol_e20", "name": "แก๊สโซฮอล์ E20", "price": 38.10, "unit": "บาท/ลิตร"},
            {"key": "gasohol_e85", "name": "แก๊สโซฮอล์ E85", "price": 34.04, "unit": "บาท/ลิตร"},
            {"key": "benzene_95",  "name": "เบนซิน 95",        "price": 50.99, "unit": "บาท/ลิตร"},
